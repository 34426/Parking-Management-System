...
public class StallServiceImpl extends ServiceImpl<StallMapper, Stall> implements StallService {
    ...
    public Object orderStall(Integer uid, Integer sid) {
        ...
        Integer count = stallMapper.selectCount(new QueryWrapper<Stall>().eq("user_id", uid));
        if(count != 0) {
            // 当前车辆已停车，无法重复停车
            return false;
        }
    ...
    public Object updateStall(Stall stall) {
        Map<String,Object> res = new HashMap<>();
        Stall stall1 = stallMapper.selectOne(new QueryWrapper<Stall>().eq("stall_num",stall.getStallNum()).eq("stall_area",stall.getStallArea()).ne("sid", stall.getSid()));
        if(SaFoxUtil.isEmpty(stall1)){
